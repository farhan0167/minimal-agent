...
A minimal async agent framework: an **agent loop** drives an LLM that can call tools, with the LLM details abstracted behind a provider-agnostic facade. The four pillars are the agent loop, the context-source system, the tool system, and the LLM facade.
...
Sessions are created through the agent's factories — `agent.create_session()` / `agent.load_session()` — which pass identity (behavior prompt + all context sources), never a rendered prompt, and forward model/backend from the agent's LLM. `load_prompt()` ([src/minimal_agent/agent/agent.py](src/minimal_agent/agent/agent.py)) resolves `str | Path | None` to the behavior prompt; the default ([agent/defaults/behavior.md](src/minimal_agent/agent/defaults/behavior.md), no `prompt` arg) is the built-in software engineering agent and auto-includes `GitStatusSource` + `DirectoryTreeSource`. Custom prompts get no context sources by default (blank slate). `WorkspaceSource` is prepended unconditionally either way. See [.claude/specifications/agent-session-factories.md](.claude/specifications/agent-session-factories.md) and [.claude/specifications/lazy-prompt-assembly.md](.claude/specifications/lazy-prompt-assembly.md).

**Context sources & placement.** [src/minimal_agent/context_sources/](src/minimal_agent/context_sources/) defines the `ContextSource` protocol (structural typing, no inheritance required), the `Placement` enum, and the built-in sources. `base.py` holds the protocol/enum/resolver helpers plus `build_context_blocks()` (formats gathered output into tagged XML blocks); each built-in source lives in its own module (`agents_md.py`, `workspace_source.py`, `git_status.py`, `directory_tree.py`, `skills.py`) and the package `__init__.py` re-exports them all, so callers import from `minimal_agent.context_sources` regardless of layout. A source's class-level `placement` declares when it's gathered and where its output lands: `SESSION` (once at the context's first `assemble()`, cached for its lifetime, rendered into the system message — the default; `WorkspaceSource`, `DirectoryTreeSource`, `SkillsContextSource`), `RUN` (once per `Agent.run()`, merged into the run's user message — `GitStatusSource`), or `CALL` (before every LLM call, standalone tail carrier — no built-in uses it). SESSION gathering is fail-fast (a raising source propagates out of the first gather); RUN/CALL failures degrade to a skipped block. Injected content is never persisted to the transcript. See [.claude/specifications/fresh-context-sources.md](.claude/specifications/fresh-context-sources.md).

**Context & storage.** `Context` ([src/minimal_agent/agent/context.py](src/minimal_agent/agent/context.py)) is the agent's interface to conversation state. It composes a `MessageStore` ([src/minimal_agent/agent/message_store.py](src/minimal_agent/agent/message_store.py)) with a static behavior prompt, context sources (partitioned by placement), and a projection strategy (default: return all messages). It is the single assembly point for what the LLM sees, across all three cadences: the async idempotent `ensure_session_gathered()` takes the SESSION snapshot at first use (called by `Agent.run()` before emitting `run.start`, and by `assemble()` as a self-heal; it doubles as the host preview trigger — await it, then read the sync `system_prompt` property); the loop calls `context.begin_run()` once per run and `await context.assemble()` before each LLM call; `assemble()` renders the system message (behavior prompt + cached SESSION blocks) and injects live-source blocks per the merge rule. `get_messages()` stays sync and pure (no I/O, no gathering) — before the first gather its system message is the behavior prompt alone. `MessageStore` is append-only; when constructed with a path, each append writes a JSONL line to disk. See [.claude/specifications/lazy-prompt-assembly.md](.claude/specifications/lazy-prompt-assembly.md).

**Sessions.** `Session` ([src/minimal_agent/agent/session.py](src/minimal_agent/agent/session.py)) is the user-facing root of a conversation: a `SessionMeta` dataclass (identity, model, backend, timestamps, usage, workspace root), the root scope's `Context`, and automatic usage rollup (it subscribes to the root scope's totals — hosts never call an accounting method). Persistence policy lives in `SessionManager` (same file): base dir, sink wiring, `create_session()` / `load_session()` / `read_meta()` / `list_sessions()`. `load_session()` validates that model/backend match the original session. The Agent holds a manager (`Agent(..., session_manager=...)`, default records under `.minimal_agent/sessions`) and its factories delegate to it — prefer those, they keep prompt, settings, and live sources consistent by construction.

**Scopes & observability.** `Scope` ([src/minimal_agent/agent/scope.py](src/minimal_agent/agent/scope.py)) is a node in the session's recording tree: an `EventEmitter` + `MessageStore` + `child()` for spawning nested recorded agents. It is *plumbing* — how a session records itself — and reaches extension code only as what `session.spawn()` yields. The session root is a `RecordedScope` (sinks: `TraceSink` → `events.jsonl`, `CallLogSink` → `calls.jsonl` + shared session-root `blobs/`, `ScopeMetaSink` → usage/fingerprint); a bare `Context()` gets a `NullScope` (zero-sink emitter, in-memory store) so there are no `None` checks anywhere. A tool that runs its own agent opens `with ctx.session.spawn(spawned_by=..., task=..., tool_call_id=ctx.tool_call_id) as scope:` — this allocates `agents/a-<id>/` under the session with the identical artifact kit, emits `agent.spawn`/`agent.end` on the parent's trace (truthful status even on crash/cancel, via the CM exit), writes `agent.json` parentage, and forwards the child's usage to the session totals. `spawn_agents` is the reference implementation. Concurrency safety across sibling scopes is isolation (per-scope emitters and files), not locking. Events live in [src/minimal_agent/events.py](src/minimal_agent/events.py); emission is fire-and-forget and can never break a run.

**The session, from inside (`SessionView`).** `SessionView` ([src/minimal_agent/agent/view.py](src/minimal_agent/agent/view.py)) is what code *running within* a session sees, as opposed to the host's outside view (`Session`/`SessionManager`) or the recording plumbing (`Scope`). One view is minted alongside every `Context` (in `Scope.new_context()`) and threaded to both extension surfaces: tools get it as `ToolContext.session`, context sources as the `gather(session)` argument. Its facets are each the narrowest interface that serves its consumer: `id`; `workspace_root` (where the agent *acts*); `transcript` (a read-only `Sequence[Message]` over the store, with a `tool_calls(name=...)` helper — no `append`, because writing the conversation is the loop's job exclusively); `events` (the emitter, fire-and-forget); `state_dir` (where the agent *remembers*); and `spawn()`. **All session-specific data reaches tools and sources through the view, per call — never through their constructors**, which would be agent-level state shared across every session that agent serves (one Agent serves N sessions, serially via resume and concurrently in the server). Tools and sources are therefore stateless objects, constructed once. `state_dir` is `<session_dir>/state/` for a recorded session (re-attached by `load_session()`, so a note written in run 1 is readable in run 40) and a lazily minted tempdir for a bare context — always a real writable directory, so no consumer branches on `None`. It is **user land**: the framework never reads, writes, migrates, or garbage-collects anything under it, and never puts its own artifacts there. See [.claude/specifications/session-env.md](.claude/specifications/session-env.md).

### System prompt

There is no system-prompt module: the prompt is assembled lazily by `Context` from two layers — the static **behavior prompt** (the agent's identity, resolved once by `load_prompt()` in `agent/`) and the cached **SESSION blocks** (gathered at the context's first `assemble()`, snapshot-preambled, `WorkspaceSource`'s `<workspace>` block first). The former `system_prompt/` package was dissolved into its callers; see [.claude/specifications/lazy-prompt-assembly.md](.claude/specifications/lazy-prompt-assembly.md). `run.start` always carries the fully rendered prompt because `Agent.run()` gathers before emitting it.
