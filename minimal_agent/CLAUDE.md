...
There is no system-prompt module: the prompt is assembled lazily by `Context` from two layers — the static **behavior prompt** (the agent's identity, resolved once by `load_prompt()` in `agent/`) and the cached **SESSION blocks** (gathered at the context's first `assemble()`, snapshot-preambled, `EnvSource`'s `<env>` block first). The former `system_prompt/` package was dissolved into its callers; see [.claude/specifications/lazy-prompt-assembly.md](.claude/specifications/lazy-prompt-assembly.md). `run.start` always carries the fully rendered prompt because `Agent.run()` gathers before emitting it.
